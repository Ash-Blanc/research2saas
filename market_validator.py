...
class ValidationStatus(Enum):
    ...
    CLEAR_OPPORTUNITY = "clear_opportunity"  # No competition, strong signals
    CROWDED_MARKET = "crowded_market"  # Many existing solutions
    EMERGING_SPACE = "emerging_space"  # Some activity, room for innovation
    NEEDS_RESEARCH = "needs_research"  # Unclear, needs more investigation
    HIGH_RISK = "high_risk"  # Red flags detected
...
class CompetitorAnalysis:
    ...
    similarity_score: float  # 0-1, how similar to proposed idea
...
class MarketValidation:
    ...
    # IP landscape
    relevant_patents: List[PatentInfo]
    patent_risk_level: str  # "low", "medium", "high"
    
    # Funding signals
    recent_funding: List[FundingSignal]
    funding_trend: str  # "increasing", "stable", "declining"
    ...
    def to_dict(self):
        return {
            ...
            "patent_risk": self.patent_risk_level,
...
class MarketValidator:
    ...
    async def validate_idea(self, 
                           idea: str,
                           paper_context: Optional[List[Dict]] = None) -> MarketValidation:
        """
        ...
        return MarketValidation(
            ...
            search_results=[],  # Populated by search tool
    ...
    async def _check_patents(self, idea: str) -> List[PatentInfo]:
        ...
        for query in search_queries:
            ...
            for result in results:
                url = result.get("url", "")
                # Check if it's a patent database result
                if any(db in url for db in ["patents.google.com", "uspto.gov"]):
                    patents.append(PatentInfo(
                        ...
                        assignee="Unknown",  # Would need to fetch full patent
                        filing_date="Unknown",
                        status="Unknown",
            ...
            for result in results:
                ...
                if any(term in snippet for term in ["raises", "funding", "million", "series"]):
                    funding_signals.append(FundingSignal(
                        ...
                        investors=[],  # Would need full article
    ...
    async def _estimate_market_size(self, idea: str) -> Dict[str, str]:
        ...
        for result in results:
            ...
            if size_match and "market_size" not in market_info:
    ...
    def _determine_status(self,
                         ...
                         red_flags: Dict) -> ValidationStatus:
        ...
        if len(funding) > 5:
            ...
            # Could be too early or no market
            return ValidationStatus.NEEDS_RESEARCH
        
        # Few competitors, some validation signals
    ...
    def _identify_strengths(self,
                          ...
                          competitors: List[CompetitorAnalysis],
                          market_info: Dict,
                          paper_context: Optional[List[Dict]]) -> List[str]:
        ...
        if market_info.get("size"):
            strengths.append(
                f"Large market opportunity: {market_info['size']}"
            )
        
        if market_info.get("growth"):
            strengths.append(
                f"Growing market: {market_info['growth']}"
            )
    ...
    def _identify_risks(self,
        ...
        if leaders:
            risks.append(
                f"Established market leaders: {', '.join(c.name for c in leaders[:3])}"
            )
        ...
        for category, flags in red_flags.items():
            if flags:
                risks.append(
                    f"{category.title()} concerns: {len(flags)} issues found"
                )
        ...
        if status == ValidationStatus.CLEAR_OPPORTUNITY:
            ...
            if competitors:
                ...
                recommendations.append(
                    f"Study {top_competitor.name}'s approach and find gaps"
                )
    ...
    def _analyze_funding_trend(self, funding: List[FundingSignal]) -> str:
        ...
        # Simple heuristic: more recent funding = increasing
        recent_funding = [
            f for f in funding
    ...
    def _extract_company_name(self, result: Dict) -> str:
        ...
        title = result.get("title", "")
        # Simple heuristic: first proper noun before "raises" or "secures"
...
if __name__ == "__main__":
    ...
    # Example validation (without web search tool)
    idea = "AI-powered code review tool for detecting security vulnerabilities"
    
    # In production, this would be async with real web search
    print(f"Validating idea: {idea}")
    print("Note: This example doesn't have web_search_tool, so results will be limited")